...
# ─── ارسال پیام با دکمه‌های رنگی از طریق API مستقیم ───
def send_colored_message(chat_id, text, reply_markup_json, parse_mode="HTML"):
    """ارسال پیام با دکمه‌های رنگی از طریق Bot API مستقیم"""
...
def blue_btn(text, callback_data):
    return {"text": text, "callback_data": callback_data, "style": "primary"}

def url_btn(text, url):
    return {"text": text, "url": url}

def inline_kb(*rows):
    """ساخت inline keyboard از ردیف‌های دکمه"""
    return {"inline_keyboard": list(rows)}


# ─── منوی اصلی (ReplyKeyboard) ───
def main_menu():
...
def cmd_start(message):
    ...
    welcome = (
        ...
        f"🏆 <b>هر {REFERRAL_REWARD_EVERY} نفر = {REFERRAL_REWARD_GB} گیگابایت رایگان هدیه!</b>\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━\n"
        "👇 از منو زیر شروع کن:"
    )
...
def handle_private(message):
    ...
    if message.content_type == 'text':
        ...
        if txt == "👨‍💻 پشتیبانی":
            kb = inline_kb(
                [url_btn("💬 ارتباط با پشتیبانی", f"https://t.me/{ADMIN_USERNAME}")]
            )
...
def handle_config_name(message, uid):
    ...
    if wallet >= final_price:
        rows.append([green_btn(f"💰 پرداخت از کیف پول ({price_fmt(wallet)} موجودی)", "pay_wallet")])
...
def cb_pay_wallet(call):
    ...
    try:
        ...
        kb = inline_kb([green_btn("🛒 خرید مجدد", "back_plans")])
        edit_colored_message(
            call.message.chat.id, call.message.message_id,
...
def cb_back(call):
    ...
    if dest == "main":
        ...
        bot.send_message(call.message.chat.id, "🏠 به منوی اصلی برگشتید.", reply_markup=main_menu())
