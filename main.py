...
# ══════════════════════════════════════════════
# لایه سازگارکننده هوشمند ارسال پیام و دکمه‌ها (حل خطای Style تلگرام)
# ══════════════════════════════════════════════
...
def _convert_markup(reply_markup):
    """تبدیل آرایه‌های دستی قدیمی به ساختار استاندارد الی‌بات"""
    if isinstance(reply_markup, dict) and "inline_keyboard" in reply_markup:
        ...
        if not reply_markup.keyboard:
            return None
        return reply_markup
    return reply_markup

def send_colored_message(chat_id, text, reply_markup_json, parse_mode="HTML"):
    markup = _convert_markup(reply_markup_json)
    try:
        msg = bot.send_message(chat_id, text, parse_mode=parse_mode, reply_markup=markup)
...
def edit_colored_message(chat_id, message_id, text, reply_markup_json, parse_mode="HTML"):
    markup = _convert_markup(reply_markup_json)
    try:
        bot.edit_message_text(text, chat_id, message_id, parse_mode=parse_mode, reply_markup=markup)
    except Exception as e:
...
def _create_and_send_config(uid, first_name, username, st, plan, final_price, wallet_paid=False):
    """ساخت کانفیگ در پاسارگاد و ارسال خودکار به کاربر"""
...
# ══════════════════════════════════════════════
# ریپلای ادمین در گروه (تایید و ساخت خودکار)
# ══════════════════════════════════════════════
@bot.message_handler(
