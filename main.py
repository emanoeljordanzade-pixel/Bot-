...
def cb_plan(call):
    ...
    if not plan:
        bot.send_message(call.message.chat.id, "❌ پلن یافت نشد!")
        return
...
def cb_pay_wallet(call):
    ...
    if not st:
        bot.answer_callback_query(call.id, "❌ خطا، دوباره شروع کنید.", show_alert=True)
        return
    ...
    if user['wallet'] < final_price:
        bot.answer_callback_query(call.id, "❌ موجودی کافی نیست! لطفاً کیف پول را شارژ کنید.", show_alert=True)
        return
    ...
    try:
        sent = bot.send_message(GROUP_ID,
            ...
            f"📦 پلن: <b>{plan['name']}</b> — {price_fmt(final_price)}\n"
        ...
        print(f"[ERROR wallet group notify] {e}")
...
def cb_pay_card(call):
    ...
    st = user_states.get(uid, {})
    if not st:
        bot.send_message(call.message.chat.id, "❌ خطا، دوباره شروع کنید.")
        return
...
def handle_purchase_receipt(message, uid):
    ...
    try:
        ...
        bot.send_message(uid,
            ...
            "⏳ ادمین کانفیگ شما را به زودی ارسال می‌کند.\n\n"
            "🙏 ممنون از خرید شما!",
            parse_mode="HTML", reply_markup=main_menu())
    except Exception as e:
...
def cb_wallet_balance(call):
    ...
    markup.add(
        types.InlineKeyboardButton("➕ شارژ کیف پول", callback_data="wallet_charge"),
        types.InlineKeyboardButton("🔙 بازگشت", callback_data="back_wallet")
    )
...
def cb_wallet_charge(call):
    ...
    bot.edit_message_text("➕ <b>شارژ کیف پول</b>\n\n"
...
def handle_wallet_receipt(message, uid):
    ...
    try:
        ...
        st['state'] = 'done'
        bot.send_message(uid, "✅ <b>رسید شارژ ثبت شد!</b>\n\n⏳ پس از تایید ادمین، موجودی اضافه می‌شود.", parse_mode="HTML", reply_markup=main_menu())
    except Exception as e:
...
def cb_reconfig(call):
    ...
    if not purchase['config_data']:
        bot.answer_callback_query(call.id, "⏳ کانفیگ هنوز آماده نشده!", show_alert=True)
        return
...
def handle_group_reply(message):
    ...
    if purchase_id is not None:
        ...
        wallet_paid = purchase.get('wallet_paid', False)

        try:
            _send_config_to_user(uid, purchase_id, config_link, plan_name, config_name, final_price, wallet_paid)
            check_referral_reward(uid, plan_name, config_name)
            bot.reply_to(message, f"✅ کانفیگ با موفقیت به کاربر <code>{uid}</code> ارسال شد.", parse_mode="HTML")
        except Exception as e:
...
def cmd_broadcast(message):
    ...
    bot.send_message(ADMIN_ID, "📢 <b>پیام همگانی</b>\n\n"
...
def handle_broadcast_message(message):
    ...
    bot.send_message(ADMIN_ID, f"⏳ در حال ارسال پیام به {len(users)} کاربر...")

    success = 0
    for u in users:
        try:
            if message.content_type == 'text':
                bot.send_message(u['uid'], message.text, parse_mode="HTML")
            elif message.content_type == 'photo':
                bot.send_photo(u['uid'], message.photo[-1].file_id, caption=message.caption, parse_mode="HTML")
            elif message.content_type == 'video':
                bot.send_video(u['uid'], message.video.file_id, caption=message.caption, parse_mode="HTML")
            success += 1
        except:
            pass
