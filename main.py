...
def handle_private(message):
    ...
    if message.content_type == 'photo':
        ...
        elif state == 'waiting_broadcast' and uid == ADMIN_ID:
            handle_broadcast(message)
        else:
...
def cmd_broadcast(message):
    ...
    bot.send_message(message.chat.id,
        "📢 <b>ارسال پیام به همه کاربران</b>\n\n"
        "پیام مورد نظر را ارسال کنید:",
        parse_mode="HTML")

@bot.message_handler(
    func=lambda m: m.chat.type == 'private' and
                   m.from_user.id == ADMIN_ID and
                   user_states.get(m.from_user.id, {}).get('state') == 'waiting_broadcast'
)
def handle_broadcast(message):
    ...
    success = 0
    failed = 0
    bot.send_message(message.chat.id, f"⏳ در حال ارسال به {len(users)} کاربر...")
    for user in users:
        try:
            bot.copy_message(user['uid'], message.chat.id, message.message_id)
            success += 1
        except:
            failed += 1
    bot.send_message(message.chat.id,
...
if __name__ == "__main__":
    db.init_db()
    print("🤖 Bot started...")
    bot_thread = Thread(target=lambda: bot.infinity_polling(timeout=60, long_polling_timeout=60, allowed_updates=["message", "callback_query", "my_chat_member"]), daemon=True)
    bot_thread.start()
    app.run(host='0.0.0.0', port=7860)
