...
def edit_msg(chat_id, message_id, text, kb=None, parse_mode="HTML"):
...
def cmd_start(message):
    ...
    text = (
        ...
        f"  ➡️ تو <b>{REFERRAL_REFERRER_DISCOUNT}٪ تخفیف</b> روی خرید بعدیت\n\n"
    ...
    if message.content_type == 'text':
        ...
        if txt == "👨‍💻 پشتیبانی":
            send_msg(message.chat.id,
                ...
                kb([url_btn("💬 ارتباط با پشتیبانی", f"https://t.me/{ADMIN_USERNAME}")]))
            return
...
def show_plans(chat_id, uid):
    user = db.get_user(uid)
    has_ref = user['referred_by'] is not None
    ...
    for key, plan in PLANS.items():
        ...
        rows.append([green(label, f"plan_{key.split('_',1)[1]}")])

    rows.append([blue("🔙 بازگشت", "back_main")])
...
def cb_plan(call):
    ...
    edit_msg(call.message.chat.id, call.message.message_id,
        ...
        kb([blue("🔙 بازگشت به پلن‌ها", "back_plans")])
    )
...
def handle_config_name(message, uid):
    ...
    rows = []
    # کیف پول - همیشه نشون بده
    if wallet >= final_price:
        rows.append([green(f"💰 کیف پول ({price_fmt(wallet)}) ✅", "pay_wallet")])
    elif wallet > 0:
        rows.append([green(f"💰 کیف پول ({price_fmt(wallet)}) — کمبود {price_fmt(final_price - wallet)}", "pay_wallet_low")])
    else:
        rows.append([green("💰 کیف پول (موجودی ندارید)", "pay_wallet_empty")])

    rows.append([green("💳 پرداخت کارت به کارت", "pay_card")])
    rows.append([blue("🔙 بازگشت", "back_plans")])
...
@bot.callback_query_handler(func=lambda c: c.data in ("pay_wallet_low", "pay_wallet_empty"))
def cb_wallet_not_enough(call):
    if call.data == "pay_wallet_empty":
        bot.answer_callback_query(call.id, "❌ موجودی کیف پول شما صفر است!", show_alert=True)
    else:
        bot.answer_callback_query(call.id, "❌ موجودی کافی نیست! لطفاً اول کیف پول را شارژ کنید.", show_alert=True)
...
def check_referral_reward(buyer_uid, plan_name, config_name):
    ...
    try:
        bot.send_message(referrer_uid,
            ...
            f"👥 تعداد دعوت‌های موفق: <b>{referral_count}</b>\n"
        ...
        if current_set > rewarded_sets:
            ...
            try:
                bot.send_message(GROUP_ID,
                    ...
                    "⬇️ ادمین ریپلای کند تا کانفیگ جایزه ارسال شود.",
                    parse_mode="HTML")
                bot.send_message(referrer_uid,
...
def show_wallet(chat_id, uid):
    ...
    send_msg(chat_id,
        ...
        kb(
            [green("💰 مشاهده موجودی", "wallet_balance"), green("➕ شارژ کیف پول", "wallet_charge")],
            [blue("🔙 بازگشت", "back_main")]
        )
    )
...
def cb_wallet_balance(call):
    ...
    edit_msg(call.message.chat.id, call.message.message_id,
        ...
        kb(
            [green("➕ شارژ کیف پول", "wallet_charge")],
            [blue("🔙 بازگشت", "back_wallet")]
        )
    )
    bot.answer_callback_query(call.id)
...
def show_account(chat_id, uid):
    ...
    rows = []
    for p in purchases:
        rows.append([green(f"📦 {p['config_name']} — {p['plan_name']}", f"reconfig_{p['id']}")])
    rows.append([blue("🔙 بازگشت", "back_main")])
